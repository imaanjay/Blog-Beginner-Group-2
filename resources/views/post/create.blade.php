<x-app-layout>
    ...
    <div class="bg-white max-w-7xl mx-auto border-2 rounded-md mt-6 p-6">
        <form method="post" action="{{ route('post.store') }}" enctype="multipart/form-data"
            class="flex flex-col w-3/4 py-3 px-8">
            @csrf
            {{-- Title --}}
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                @if ($categories->isEmpty())
                    <h1 class="text-red-600 block mb-2 text-sm font-medium">Tidak ada kategori yang ditemukan</h1>
                @else
                    <select name="category_id" id="category_id" class="w-1/4 rounded-md">
            ...
            <!-- Selected Tag -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Selected Tags</label>
            ...
            <div class="mb-3">
                ...
                @error('full_text')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
                <input id="full_text" type="hidden" name="full_text" value="{{ old('full_text') }}" />
                <trix-editor input="full_text"></trix-editor>
            </div>

            <button type="submit" {{ $categories->isEmpty() ? 'disabled' : '' }}
